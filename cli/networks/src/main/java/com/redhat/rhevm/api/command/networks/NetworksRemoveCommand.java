 ...
 * Removes a Network
 */
@Command(scope = "networks", name = "remove", description = "Removes a Network.")
public class NetworksRemoveCommand extends AbstractRemoveCommand {

    @Argument(index = 0, name = "name", description = "The name of the Network", required = true, multiValued = false)
    protected String name;

    protected Object doExecute() throws Exception {
        doRemove(client.getCollection("networks", Networks.class).getNetworks(), Network.class, name);
        return null;
    }
}
