 ...
 * Update a Cluster
 */
@Command(scope = "clusters", name = "update", description = "Update Cluster state")
public class ClustersUpdateCommand extends AbstractUpdateCommand<Cluster> {

    @Argument(index = 0, name = "name", description = "The name of the Cluster", required = true, multiValued = false)
    protected String name;

    protected Object doExecute() throws Exception {
        doUpdate(client.getCollection("clusters", Clusters.class).getClusters(), Cluster.class, "cluster", name);
        return null;
    }
}
